...
local function generate_line(line)
    ...
    table.insert(text,
                 string.format(' begin="%s" end="%s"', start_time, end_time))
...
local function generate_translations()
    ...
    for lang, lines in pairs(trans) do
        table.insert(text, string.format(
                         [[<translation type="subtitle" xml:lang="%s">]], lang))
        for i = 1, #lines do
            local line = lines[i]
    ...
    if #text > 0 then
        return [[<translations>]] .. table.concat(text) .. [[</translations>]]
    else
        return [[<translations />]]
    end
end
    ...
    for lang, roma in pairs(roman) do
        if #roma.lines ~= 0 then
            ...
            for i = 1, #roma.lines do
                ...
                table.insert(temp, string.format([[<text for="L%d">]], line.L))
...
local function generate_head()
    ...
    return
        '<head><metadata>' .. table.concat(ttml) .. generate_iTunesMetadata() ..
            '</metadata></head>'
end
...
local function collect_roma(main_syls, roma_line)
    local function insert_roma(syl)
        ...
        if primary ~= nil then
            main_syls[primary].roma = (main_syls[primary].roma or "") ..
                                          syl.text_stripped
            main_syls[primary].furi = main_syls[primary].furi - 1
        elseif secondary ~= nil then
            main_syls[secondary].roma = (main_syls[secondary].roma or "") ..
                                            syl.text_stripped
